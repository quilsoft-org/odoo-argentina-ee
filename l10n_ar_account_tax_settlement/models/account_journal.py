...
def get_line_tax_base(move_line):
    return sum(
        move_line.move_id.line_ids.filtered(
            lambda x: move_line.tax_line_id in x.tax_ids
        ).mapped("balance")
    )
...
class AccountJournal(models.Model):
    ...
    settlement_tax = fields.Selection(
        selection_add=[
            ...
            (
                "iibb_aplicado",
                "TXT Perc/Ret IIBB aplicadas ARBA: Percepciones "
                "( excepto actividad 29, 7 quincenal, 7 y 17 de Bancos)",
            ),
            (
                "iibb_aplicado_act_7",
                "TXT Perc/Ret IIBB aplicadas ARBA: Percepciones Act. "
                "7 método Percibido (quincenal)",
            ),
    ...
    def check_withholding_autmatic_installed(self):
        ...
        if not account_withholding_automatic and any(
            self.filtered(
                lambda x: x.settlement_tax in ["iibb_aplicado_api", "sicore_aplicado"]
            )
        ):
    ...
    def iibb_aplicado_dgr_mendoza_files_values(self, move_lines):
        ...
        for line in move_lines:
            ...
            if not alicuot_line:
                raise ValidationError(
                    _('No hay alicuota configurada en el partner "%s" (id: %s)')
            ...
            content += (payment.withholding_number or "").rjust(12, "0")[
                :12
            ]  # we are forcing 12 first numbers always.
        ...
        periodo = (
            fields.Date.from_string(move_line.date).strftime("%Y") or ""
        )  # 'pppp' AÑO '2020'
    ...
    def _get_perception_original_invoice_number(self, line):
        ...
        res += "%012d" % int(
            re.sub("[^0-9]", "", related_invoice.l10n_latam_document_number or "")
        )
        return res
    ...
    def iibb_aplicado_api_files_values(self, move_lines):
        ...
        def format_amount(amount, integers, decimals=2):
            ...
            # "{0:>16.2f}".format(12.1)
            return template % "{0:.2f}".format(round(amount, decimals)).replace(
                ".", ","
            )
        ...
        for line in move_lines:
            ...
            # 1 - tipo de operacion
            if (
                tax.type_tax_use in ["sale", "purchase"]
                and tax.amount_type == "partner_tax"
            ):
                content = "2"
                ...
                articulo_inciso_calculo = (
                    alicuot_line.api_articulo_inciso_calculo_percepcion or "000"
                )
                articulo_inciso_retiene = alicuot_line.api_codigo_articulo_percepcion
            elif (
                tax.type_tax_use in ["customer", "supplier"]
                and tax.withholding_type == "partner_tax"
            ):
                content = "1"
                ...
                articulo_inciso_retiene = alicuot_line.api_codigo_articulo_retencion
            else:
                raise ValidationError(
                    _(
            ...
            content += (re.sub("[^0-9]", "", partner.gross_income_number or "")).rjust(
                10, "0"
            )
            ...
            if line.move_id:
                ...
                vat_amount = 0.0
                base_amount = (
                    line.payment_id and line.payment_id.withholdable_base_amount or 0.0
                )
    ...
    def iibb_aplicado_agip_files_values(self, move_lines):
        ...
        for line in move_lines.sorted("date"):
            ...
            if internal_type == "credit_note":
                ...
                content += fields.Date.from_string(or_inv.invoice_date).strftime(
                    "%d/%m/%Y"
                )
            ...
            if payment_group:
                ...
                total_amount = (
                    1 if line.move_id.is_inbound() else -1
                ) * line.move_id.amount_total_signed
    ...
    def iibb_aplicado_sircar_files_values(self, move_lines):
        ...
        for line in move_lines.filtered("payment_id"):
            ...
            content.append(format_amount(payment.withholdable_base_amount, 12, 2, "."))

            # 8 alicuota de la retencion
            content.append(format_amount(alicuot_line.alicuota_retencion, 6, 2, "."))
            ...
            if not line.tax_line_id.jurisdiction_code:
                raise ValidationError(_("No hay etiqueta de jurisdicción configurada!"))
            ...
            if line.tax_line_id.jurisdiction_code == "904":
                ...
                content.append(
                    "%014d"
                    % int(re.sub("[^0-9]", "", original_invoice.document_number or ""))
                    if internal_type == "credit_note"
                    else "%014d" % 0
                )
        ...
        for line in move_lines.filtered(lambda x: x.move_id.is_invoice()):
            ...
            content.append(format_amount(alicuot_line.alicuota_percepcion, 6, 2, "."))
    ...
    def sicore_aplicado_files_values(self, move_lines):
        ...
        for line in move_lines.sorted(key=lambda r: r.date):
            ...
            content += fields.Date.from_string(pay_group.payment_date).strftime(
                "%d/%m/%Y"
            )
            ...
            if line.tax_line_id.tax_group_id in (
                ...
                content += (
                    regimen
                    and "%03d"
                    % int("".join(filter(str.isdigit, str(regimen.codigo_de_regimen))))
                    or "000"
                )
                ...
                raise ValidationError(
                    _("Para sicore solo IVA y Ganancias están implementados")
                )
            ...
            content += fields.Date.from_string(payment.payment_date).strftime(
                "%d/%m/%Y"
            )
