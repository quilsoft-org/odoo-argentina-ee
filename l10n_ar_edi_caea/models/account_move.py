...
class AccountMove(models.Model):
    ...
    l10n_ar_afip_pos_system = fields.Char(
        string="AFIP POS System", compute="_compute_l10n_ar_afip_pos_system"
    )

    l10n_ar_afip_result = fields.Selection(
        selection_add=[("R", "Rejected")], ondelete={"R": "set null"}
    )

    caea_id = fields.Many2one("l10n_ar.afipws.caea", string="Caea", copy=False)
    ...
    def _post(self, soft=True):
        ...
        if caea_state == "active":
            for inv in self.filtered(
                lambda x: (
                    x.journal_id.l10n_ar_afip_pos_system == "CAEA"
                    or x.journal_id.caea_journal_id
                )
    ...
    def wsfe_get_caea_request(self, caea, client=None):
        ...
        vat = (
            partner_id_code
            and self.commercial_partner_id.vat
            and re.sub(r"\D+", "", self.commercial_partner_id.vat)
        )
            ...
            "FeDetReq": [
                {
                    "FECAEADetRequest": {
                        ...
                        "ImpTotConc": float_repr(
                            amounts["vat_untaxed_base_amount"], precision_digits=2
                        ),  # Not Taxed VAT
                        "ImpNeto": float_repr(
                            amounts["vat_taxable_amount"], precision_digits=2
                        ),
                        "ImpOpEx": float_repr(
                            amounts["vat_exempt_base_amount"], precision_digits=2
                        ),
                        "ImpTrib": float_repr(
                            amounts["not_vat_taxes_amount"], precision_digits=2
                        ),
                        "ImpIVA": float_repr(amounts["vat_amount"], precision_digits=2),
                        # Service dates are only informed when AFIP Concept is (2,3)
                        "FchServDesde": service_start.strftime(WS_DATE_FORMAT["wsfe"])
                        if service_start
                        else False,
                        "FchServHasta": service_end.strftime(WS_DATE_FORMAT["wsfe"])
                        if service_end
                        else False,
                        "FchVtoPago": due_payment_date.strftime(WS_DATE_FORMAT["wsfe"])
                        if due_payment_date
                        else False,
                        "MonId": self.currency_id.l10n_ar_afip_code,
                        "MonCotiz": float_repr(
                            self.l10n_ar_currency_rate, precision_digits=6
                        ),
                        "CbtesAsoc": ArrayOfCbteAsoc([related_invoices])
                        if related_invoices
                        else None,
                        ...
                        "Compradores": None,
                        "CAEA": caea,
                        "CbteFchHsGen": self.caea_post_datetime.strftime(
                            "%Y%m%d%H%M%S"
                        ),
                    }
                }
    ...
    def _get_related_invoice_data(self):
        ...
        if int(self.l10n_latam_document_type_id.code) not in [
            2,
            3,
            7,
            8,
        ...
        res.update(
            {
                wskey[afip_ws]["type"]: related_inv.l10n_latam_document_type_id.code,
                wskey[afip_ws][
                    "pos_number"
                ]: related_inv.journal_id.l10n_ar_afip_pos_number,
            ...
            res.update(
                {
                    wskey[afip_ws][
                        "cuit"
                    ]: related_inv.company_id.partner_id._get_id_number_sanitize()
                }
            )

        # WSFE_10158 send orignal invoice date on an mipyme document
        if afip_ws == "wsfe" and (
            self._is_mipyme_fce() or self._is_mipyme_fce_refund()
        ):
            res.update(
                {
                    wskey[afip_ws]["date"]: related_inv.invoice_date.strftime(
                        WS_DATE_FORMAT[afip_ws]
                    )
                }
            )
