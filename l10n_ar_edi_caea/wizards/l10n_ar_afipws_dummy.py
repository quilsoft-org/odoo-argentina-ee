...
class L10nArAfipwsDummy(models.TransientModel):
    ...
    use_caea = fields.Boolean(
        string="Allow contingency billing", related="company_id.use_caea"
    )

    afip_ws = fields.Char(string="AFIP WS", default=lambda self: self.get_afip_ws())
    ...
    def _onchange_afip_ws(self):
        ...
        try:
            client, auth, transport = (
                self.company_id.with_context(ignore_active_caea=True)
                ._l10n_ar_get_connection(self.afip_ws)
            ...
            self.write(
                {
                    "app_server_status": True if response.AppServer == "OK" else False,
    ...
    def afip_red_button(self):
        self.env["ir.config_parameter"].sudo().set_param("afip.ws.caea.state", "active")
        self.env["l10n_ar.afipws.caea.log"].sudo().create(
